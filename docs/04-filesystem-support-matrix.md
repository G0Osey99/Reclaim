...
5. Diff: a `(inode, name, parent)` present at xid *k* but absent at the newest xid ⇒ **deleted between k and now**. Report with the path from the *old* tree and the extents from the old extent records; mark each extent block as overwritten-or-not by checking the *current* space-manager bitmap (allocated now ⇒ likely overwritten ⇒ lower score).
...
7. **Orphan scan**: scan unallocated blocks for B-tree node headers (`obj_phys_t` with valid checksum and `o_type` of BTREE_NODE, subtype FSTREE) to recover records from trees that have dropped out of every checkpoint.
