...
These are the facts that shape what a Mac recovery tool can and cannot do. Items marked **[verify]** are widely reported but should be confirmed empirically on your own hardware during M0 (doc 10) before you design around them.
...
| **Full Disk Access (TCC)** governs access to protected *user folders* (Desktop, Documents, Downloads, Mail, Messages, Time Machine backups, etc.) at the *file* level, and — since macOS 10.15 — is also required for a root process to read the raw device that backs the boot volume. **[verify: exact TCC behaviour per macOS version; test `sudo dd if=/dev/rdisk0 count=1` with and without FDA granted to Terminal.]** | Ship an onboarding step that deep-links to *System Settings → Privacy & Security → Full Disk Access* and checks status by attempting a read. |
...
- When you boot the same Mac (normal mode or Recovery Mode) the storage controller decrypts transparently; raw reads of the physical store return **plaintext container blocks** if FileVault is off, or **APFS-level-encrypted volume blocks** if FileVault is on until the volume is unlocked. **[verify: after unlocking a FileVault volume in normal boot, do reads from `/dev/rdisk3s5` (the synthesized volume device) return decrypted file-system blocks? Reports from vendor docs suggest recovery tools work on the unlocked Data volume in normal boot; confirm and document exactly which device node to read.]**
...
- FileVault 2 on APFS = per-volume AES-XTS with a Volume Encryption Key wrapped by user Key Encryption Keys / recovery key. Unlock via `diskutil apfs unlockVolume` (or the mount prompt); after that, read through the OS-provided decrypted path (§4 [verify]).
...
2. **Mounted-volume source** — walk the live filesystem via POSIX for Trash, snapshots, and as a fallback when raw access is refused. Also how "recover from FileVault volume" works when [verify] answers "no".
